...
void screenForceStatusUpdate(void) {
  BYTE shells, mines, armour, trees;
  BYTE num, i;
  tankGetStats(&mytk, &shells, &mines, &armour, &trees);
  frontEndUpdateTankStatusBars(shells, mines, armour, trees);

  /* Push pill states to HUD */
  num = pillsGetNumPills(&mypb);
  for (i = 1; i <= num; i++)
    frontEndStatusPillbox(i, pillsGetAllianceNum(&mypb, i));

  /* Push base states to HUD */
  num = basesGetNumBases(&mybs);
  for (i = 1; i <= num; i++)
    frontEndStatusBase(i, basesGetStatusNum(&mybs, i));

  /* Push own tank state to HUD (player slot = self+1, 1-based) */
  frontEndStatusTank((BYTE)(playersGetSelf(screenGetPlayers()) + 1), tankSelf);

  /* Show player name in the message bar at startup */
  {
    char pname[68];
    pname[0] = '\0';
    playersGetPlayerName(screenGetPlayers(), playersGetSelf(screenGetPlayers()), pname);
    frontEndMessages(pname, "");
  }
}
...
void screenManMoveAtGunsight(buildSelect buildS) {
  BYTE gsX, gsY, pxDummy, pyDummy;
  if (tankGetArmour(&mytk) <= TANK_FULL_ARMOUR && netGetStatus() != netFailed) {
    tankGetGunsight(&mytk, &gsX, &gsY, &pxDummy, &pyDummy);
    lgmAddRequest(&mylgman, &mymp, &mypb, &mybs, &mytk, gsX, gsY, (BYTE) buildS);
  }
}
